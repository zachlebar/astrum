...
Astrum is a lightweight pattern library designed to be included with any web project. It's built by the fine folks at No Divide Studio. For more information on Astrum, checkout their repo: https://github.com/NoDivide/Fastrum.

## Table of Contents
- [Getting Started](#getting-started)
- [How Fastrum Works](#getting-started)
...
- [Updating Fastrum](#updating-Fastrum)
...
Fastrum is built on Docker to create a local dev server for easy setup. If you're not familiar with Docker, checkout their documentation: https://www.docker.com/what-docker. Once you have Docker installed (https://docs.docker.com/install/), you can follow the instructions below to setup Fastrum:

First, clone this repo into a directory on your machine. Or if you'd prefer, you can fork this repository first, then clone the repo to your local machine.

Fastrum has a command line interface used to administer the app. Using Terminal, `cd` into the directory you just cloned.

Before we can get started, we need to have Docker build our server infrastructure. Run `docker-compose up -d` to have Docker build our server infrastructure and run it in the background.

Now that we have the server up and running, we need to initialize a pattern library. The command we need to run is:
`docker-compose run --rm fastrum astrum init ./pattern-lib`

Let's break that down. `docker-compose run` is what lets us connect to the Docker container and administer the program. `--rm` tells Docker to delete the container after we've run our command. Otherwise, Docker will keep spawning new containers with each `docker-compose run` command we run. `fastrum` is the name of the Docker container we're connecting to. `astrum` is the name of the command being run in that container. `init` is the command being given to `astrum` and `./pattern-lib` is the directory we're creating our pattern library inside.

At this point, you should be able to access Fastrum by visiting http://0.0.0.0:7010/pattern-lib (or whatever directory you specified) in your browser. There's nothing there yet though.

To connect to Figma and import your components, you'll need to run the following command: `docker-compose run --rm fastrum astrum figma`. Everything about this command is the same as before, except now we're running the command `figma` through `astrum`. At this point you'll be prompted to provide your Figma Personal Access Token, as well as the URL to the Figma file you're pulling these components from. If you don't know your Figma Personal Access Token, follow the instructions found in Figma's documentation: https://www.figma.com/developers/docs#auth.

Once you've added in the correct information, you'll run the following command to fetch the Figma components and auto-generate the corresponding Fastrum components: `docker-compose run --rm fastrum astrum figma fetch`.

You can now visit http://0.0.0.0:7010/pattern-lib (or whatever directory you specified) in your browser, and you'll see the auto-generate Fastrum components, complete with Figma reference images. You can then go about your business creating the HTML and CSS for each of these components. For more information on how to do that, keep reading.

<a href=“#how-Fastrum-works”></a>
## How Fastrum Works
Fastrum is a single page, [Vue.js](https://vuejs.org) powered, app. As mentioned previously, it's comprised of components organized into groups and loads its all important data from a core `data.json` file.
...
favicon.png              // Fastrums default favicon.
...
### The data.json File
The `data.json` file is central to how Fastrum works and should be relatively self explanatory upon opening it. By default it looks like this:
...
You can specify as many creators as you need. Fastrum will automatically format these into a "created by" line that appears at the bottom of the sidebar.
...
Fastrum supports basic theming to help you bring your pattern library in line with your projects branding.
...
code_hightlight_theme | github | Fastrum uses [highlight.js](https://highlightjs.org) for its code samples. You can use any style you like: [available styles](https://highlightjs.org/static/demo)
...
show_version | true | Show the Fastrum version at the bottom of the sidebar.
...
```
Fastrum includes an introduction page by default.
...
**Note:** *If you don’t require any pages in your pattern library simply set the `content` key value to `null`.*

<a href=“#adding-components”></a>
## Adding Components
The simplest way to add a component to your pattern library is by using the Fastrum command-line tool which will ask you a series of questions on how you want the component to be configured. For example:

`astrum new navigation/primary`

![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-new.gif)
...
### Component Types
Fastrum currently supports two component types. The default component and a **colors** type. The **colors** type lets you include a color palette in your pattern library and to include it you use the `--type` option:

`astrum new branding/color-palette --type colors`

![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-new-colors.gif)
...
sample_min_height|integer|Fastrum detects if a component is hidden at desktop or mobile resolutions by detecting the components rendered height. When it’s hidden in your project CSS, Fastrum shows a message to this effect. If the component is absolutely positioned, it has no height so you can set a min-height with this option to ensure it is shown properly and Fastrum messaging is shown correctly.
sample_overflow_hidden|boolean|Apply `overflow: hidden;` to the component sample.
disabled_auto_sample_hiding|object|Fastrum automatically detects if you've hidden a component at mobile or desktop resolutions in your stylesheets. You can disable this feature using this option. Add `show_on_mobile` and `show_on_desktop` keys to the object with boolean values to set how the component should behave.
...
![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-edit.gif)
...
![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-edit-group.gif)

**Note:** *If you change a groups name, Fastrum automatically updates all of the groups components.*
...
![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-list.gif)
...
![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-delete.gif)
...
![](https://dl.dropboxusercontent.com/u/251342/Fastrum-gifs/Fastrum-delete-group.gif)
...
This doesn't mean that Fastrum cannot be used in older browsers, we’re just aiming to ensure compatibility with those mentioned above.

<a href=“#acknowledgements”></a>
## Acknowledgements
Fastrum wouldn’t work without [Vue.js](http://vuejs.org/) and the work that Evan You is doing there: [support his efforts](http://vuejs.org/support-vuejs/).
